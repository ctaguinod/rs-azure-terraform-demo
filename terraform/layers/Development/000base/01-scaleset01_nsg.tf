resource "azurerm_network_security_group" "scaleset01_nsg" {
  name                = "${var.scaleset01_subnet_name}-NSG"
  ...
  security_rule {
    ...
    destination_address_prefix = var.scaleset01_subnet_prefix
  ...
  security_rule {
    ...
    protocol                   = "*"
    source_port_range          = "*"
    source_address_prefix      = var.scaleset01_subnet_prefix
    destination_address_prefix = var.scaleset01_subnet_prefix
...
output "scaleset01_nsg_nsg_id" {
  value = "${azurerm_network_security_group.scaleset01_nsg.id}"
}
